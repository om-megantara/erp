...
<div class="content-wrapper">
	...
	<section class="content">
		      ...
		      <div class="modal-body">
		        <table>
		              ...
		              <td class="productcode"></td>
		                <!-- <td class="stock"></td> -->
		                <td>
		                <input class="opnameqty" name="opnameqty[]" value="0" required="">
		                ...
		                <input class="form-control input-sm adjustment" type="hidden" readonly="" name="adjustment[]" type="number" value="0" required=""> 
		              </td><input class="form-control input-sm diff" type="hidden" name="diff[]" value="0" readonly="" required="">
		              <!-- <td>
		...
		<div class="box box-solid">
		  <div class="box-body">
		      <form name="form" id="form" action="<?php echo base_url();?>development/stock_check_add_act" method="post" enctype="multipart/form-data" autocomplete="off">
		        <div class="col-md-6">
		          <div class="form-group">
		            ...
		            <span class="left2">
		              ...
		              <input type="hidden" class="" id="checkid" name="checkid" required="">
		            </span>
		          </div> 
		        </div>
		        <div class="col-md-6"> 
...
j8( document ).ready(function( $ ) {
	...
	if (product!=null) {
		j8("#checkid").val('<?php if ( isset($content['detail2'])){ echo $content['main']['CheckID']; }?>')
		j8("#opnameid").val('<?php if ( isset($content['detail2'])){ echo $content['main']['OpnameID']; }?>')
		j8("#date").datepicker("setDate", '<?php if ( isset($content['detail2'])){ echo $content['main']['CheckDate']; }?>');
		j8("#note").val('<?php if ( isset($content['detail2'])){ echo $content['main']['CheckNote']; }?>')
...
function openPopupOneAtATime(x) {
    if (popup && !popup.closed) {
       popup.close();
       popup = window.open('<?php echo base_url();?>general/product_list_popup_warehouse_check?id='+x,'_blank', 'width=700,height=500,left=200,top=100');     
    } else {
       popup = window.open('<?php echo base_url();?>general/product_list_popup_warehouse_check?id='+x,'_blank', 'width=700,height=500,left=200,top=100');     
    }
}
...
function fillproduct(val) {
  WarehouseID = j8(".warehouse").val();
	  ...
	  j8(".cellproduct:first .opnameqty").val(val['OpnameQty']);
...
function fillproduct2(val) {
  if (val.length > 0) {
	  for( var i = 0; i<val.length; i++){
		...
		j8(".cellproduct:first .opnameqty").val(val[i]['OpnameQty']);
...
j8("#form").submit(function(e) {
    ...
    j8('.table-main .opnameqty').each(function(){
      opnameqty = j8(this).val();
      if (opnameqty < 0) {
        e.preventDefault();
        alert("Input 'Stock' Minimal 0")
        return false
      }
    });
});
