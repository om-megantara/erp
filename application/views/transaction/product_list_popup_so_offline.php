        ...
        <table id="dt_list" class="table table-bordered table-hover nowrap" width="100%">
          <thead>
            <tr>
              ...
              <th class=" alignCenter">Update</th>
...
j11( document ).ready(function( $ ) {
  ...
  .DataTable({
    ...
    "serverSide": true, 
    "ajax": {
      "url": "<?php echo site_url('transaction/product_list_popup_so_data_offline')?>",
      "type": "POST",
      beforeSend: function(){
...
$('.insert_data').live('click', function(e){
  var
    ...
    valueToPush.tdname  = par.find("td:nth-child(8)").html();
    valueToPush.tdcode  = par.find("td:nth-child(3)").html();
    valueToPush.tdstock = par.find("td:nth-child(4)").html();
    window.opener.ProcessChildMessage(valueToPush);
    console.log(valueToPush)
}); 
$('.cekStok').live('click', function(e){
  ProductID = $(this).attr('ProductID')
  $.ajax({
    url: "<?php echo base_url();?>transaction/cek_stock_all",
    type : 'POST',
    ...
    success : function (response) {
      ...
      for( var i = 0; i<response.length; i++){
        res += response[i]['WarehouseName']+' : '+response[i]['Quantity']+'\n'
      }
      alert(res)
