        ...
        <table id="dt_list" class="table table-bordered " style="width: 100% !important;">
          <thead>
            <tr>
              ...
              <th class="alignCenter">Date1 | PURCHASING</th>
              <th class="alignCenter">Date2 | TIM SERVICE</th>
              <th class="alignCenter">Date3</th>
              <th class="alignCenter">DateFinal</th>
            ...
            if (isset($content['main'])) {
                ...
                <tr>
                  ...
                  <td class="alignCenter">
                    <?php 
                      if ($list['SOShipDate1Need'] == 1) {
                        echo $list['SOShipDate1'].' / '.$list['userDate1'];
                      } else {
                  ...
                  <td class="alignCenter">
                    <?php 
                      if ($list['SOShipDate2Need'] == 1) {
                        echo $list['SOShipDate2'].' / '.$list['userDate2'];
                      } else {
                        echo ' <i class="fa fa-fw fa-ban"></i> ';
                      }
                    ?>
                  ...
                  <td class="alignCenter">
                    <?php 
                      if ($list['SOShipDate3Need'] == 1) {
                        echo $list['SOShipDate3'].' / '.$list['userDate3'];
                      } else {
                        echo ' <i class="fa fa-fw fa-ban"></i> ';
                      }
                    ?>
                  </td>
                  <td class="alignCenter"><?php echo $list['SOShipDateFinal'];?></td>
                  <td>
                      <button type="button" class="btn btn-success btn-xs detail" title="DETAIL" soid="<?php echo $list['SOID'];?>" data-toggle="modal" data-target="#modal-note"><i class="fa fa-fw fa-edit"></i></button>
                  </td>
                </tr>
          <?php } } ?>
          </tbody>
...
jQuery( document ).ready(function( $ ) {
  ...
  function get(soid) {
    ...
    var url="<?php echo base_url();?>transaction/sales_order_due_date_detail"
    xmlHttp.onreadystatechange=stateChanged
    xmlHttp.open("POST",url,true)
    xmlHttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
    xmlHttp.send("SOID="+soid);
  ...
  $('body').on('focus',".fieldDueDate", function(){
      $(this).datepicker({ autoclose: true, format: 'yyyy-mm-dd'});
  });
  $('.updateDueDate').live('click', function(e){
      soid  = $(this).attr("soid")
      data  = $(this).attr("data")
      user  = $(this).attr("user")
      date  = $('#'+data).val()
      ...
      $.ajax({
        url: "<?php echo base_url();?>transaction/sales_order_due_date_detail_act",
        type : 'POST',
        data: {data: data, user: user, date: date, soid: soid},
        dataType : 'json',
