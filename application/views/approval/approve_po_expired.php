            ...
            <tbody>
              ...
              ?>
                      <tr>
                        ...
                        <td>
                          <button type="button" class="btn btn-success btn-xs detail" title="DETAIL" id="<?php echo $list['ApprovalID'];?>" POID="<?php echo $list['POID'];?>" data-toggle="modal" data-target="#modal-note"><i class="fa fa-fw fa-edit"></i></button>
                        </td>
                      </tr>
              <?php } } ?>
            </tbody>
...
jQuery( document ).ready(function( $ ) {
  ...
  function get(POID, id) {
    xmlHttp=GetXmlHttpObject()
      var url="<?php echo base_url();?>approval/approve_po_detail"
      url=url+"?POID="+POID+"&id="+id+"&type=po_expired"
      // alert(url);
  ...
  $('.approve').live('click',function(e){
      POID = $(this).attr('po');
      id   = $(this).attr('id');
      data = {POID:POID, id:id};
      var r = confirm("Apa anda yakin akan me-Approve PO "+POID+"?");
      if (r == true) {
        $.ajax({
          url: "<?php echo base_url();?>approval/approve_po_expired_act/approve",
          type : 'POST',
          data : data,
  ...
  $('.reject').live('click',function(e){
      ...
      id   = $(this).attr('id');
      data = {POID:POID, id:id};
      var r = confirm("Apa anda yakin akan me-Reject PO "+POID+"?");
      if (r == true) {
        $.ajax({
          url: "<?php echo base_url();?>approval/approve_po_expired_act/reject",
          type : 'POST',
          data : data,
          success : function (response) {
            window.location.href = "<?php echo current_url(); ?>";
