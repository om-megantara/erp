...
<style type="text/css"> 
  ...
  .tooLate td:not(:first-child) { background: #ffc9c1 !important; }
  .TotalRow { background-color: #3c8dbc73 !important; }
</style>
  ...
  <section class="content">
    ...
    <div class="box box-solid">
      <div class="box-header">
        ...
        <div id="divhideshow">
          Hide/Show Column :
          <a class="btn btn-xs toggle-vis btn btn-primary btn-xs" data-column="7">Sales Name</a>
          <a class="btn btn-xs toggle-vis btn btn-primary btn-xs" data-column="8">City Count</a>
        </div>
      </div>
      <div class="box-body">
        <table id="dt_list" class="table table-bordered">
          <thead>
            <tr>
              ...
              <th class="alignCenter">Progress</th>
              <th></th>
          ...
          <?php
            ...
            if (isset($content['main'])) {
                ...
                <tr>
                  ...
                  <td class="alignRight"><?php echo number_format($list['Progress'],2);?> %</td>
                  <td>
                    <button type="button" class="btn btn-primary btn-xs detail" title="DETAIL" id="<?php echo $list['CityID']; ?>" datestart="<?php echo $list['INVDateStart']; ?>" dateend="<?php echo $list['INVDateEnd']; ?>" data-toggle="modal" data-target="#modal-detail"><i class="fa fa-fw fa-search"></i></button>
                    <!-- <button type="button" class="btn btn-success btn-xs detailmonth" title="DETAIL BY MONTH" id="<?php echo $list['CityID']; ?>" datestart="<?php echo $list['INVDateStart']; ?>" dateend="<?php echo $list['INVDateEnd']; ?>"><i class="fa fa-fw fa-search"></i></button> -->
                  </td>
...
jQuery( document ).ready(function( $ ) {
  var table = $('#dt_list').DataTable({
    ...
    fnRowCallback: function( nRow, aData, iDisplayIndex, iDisplayIndexFull ) {  
      type = aData[0].slice(0)
      Progress = aData[5].slice(0, -1)
      // console.log(type)
      if ( Progress < 0 ){
          jQuery(nRow).addClass('tooLate');
      }
      if ( type === "Total" ){
          jQuery(nRow).addClass('TotalRow');
      }
    },
  })
  ...
  function get(id, datestart, dateend) {
    xmlHttp=GetXmlHttpObject()
      var url="<?php echo base_url();?>report/report_inv_city_detail"
  ...
  function get2(id, datestart, dateend) {
    xmlHttp=GetXmlHttpObject()
      var url="<?php echo base_url();?>report/report_inv_city_detail_month"
      url=url+"?id="+id+"&datestart="+datestart+"&dateend="+dateend
      document.getElementById("detailcontent").innerHTML='<i class="fa fa-spinner fa-spin fa-3x fa-fw"></i><span class="sr-only">Loading...</span>'
      // alert(url);
      xmlHttp.onreadystatechange=stateChanged
         ...
         '<tr>'+
            $.map(table.columns().visible(),
                function(colvisible, colindex){
                   return (colvisible) ? "<th>" + $(table.column(colindex).header()).text() + "</th>" : null;
             }).join("") +
         '</tr>'+
         '</thead>'+
         '<tbody>' +
