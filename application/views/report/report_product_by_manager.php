        ...
        <table id="dt_list" class="table table-bordered " style="width: 100% !important;">
          <thead>
            <tr>
              ...
              <th class=" alignCenter">Product Manager </th>
              <th class=" alignCenter">Total Product</th>
              <th></th>
            </tr>
          </thead>
          <tbody>
          <?php
            if (isset($content['main'])) {
                ...
                <tr>
                  ...
                  <td><?php echo $list['ProductAtributeValueName'];?></td>
                  <td class="alignRight"><?php echo $list['Total'];?></td>
                  <td>
                    <button type="button" class="btn btn-primary btn-xs detail" title="DETAIL" id="<?php echo $list['AtributeValue']; ?>" data-toggle="modal" data-target="#modal-detail"><i class="fa fa-fw fa-search"></i></button>
                  </td>
                </tr>
          <?php } } ?>
          </tbody>
...
jQuery( document ).ready(function( $ ) {
  ...
  var table = $('#dt_list').DataTable({
    ...
    "footerCallback": function ( row, data, start, end, display ) {
        ...
        $( api.column( 2 ).footer() ).html(
            total2.toLocaleString(undefined, {minimumFractionDigits: 0})
        );
    }
  })
  ...
  function get(id) {
    xmlHttp=GetXmlHttpObject()
      var url="<?php echo base_url();?>report/report_product_by_manager_detail"
      url=url+"?id="+id
      // alert(url);
