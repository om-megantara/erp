    ...
    <tbody>
      <?php
        ...
        foreach ($content['category'] as $row => $list) { 
          if (isset($main[$list['ProductCategoryID']])) {
            $Total_qty += $main[$list['ProductCategoryID']]['ProductQty'];
            $Total_price += $main[$list['ProductCategoryID']]['PriceTotal'];
      ?>
            <tr>
                <td class=""><?php echo $list['ProductCategoryName'];?></td>
                <td class="alignRight"><?php echo number_format($main[$list['ProductCategoryID']]['ProductQty']);?></td>
                <td class="alignRight"><?php echo number_format($main[$list['ProductCategoryID']]['PriceTotal'],2);?></td>
            </tr>
      <?php } } ?>
