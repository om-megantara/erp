                ...
                if ($content['list']) {
                  ...
                  foreach ($content['list'] as $row => $list) { $no++;?>
                      <tr>
                        ...
                        <td><?php if($list['stock']<0){echo "0";} else {echo $list['stock'];}?></td>
                        <td><?php echo number_format($list['ProductPriceDefault']);?></td>
                        <td><?php echo $list['popending'];?></td>
                        <td><?php if($list['stock']<1 AND $list['NoteDetail']=="Add DO" ) { echo "Stock Habis"; } else if($list['stock']==1 AND $list['NoteDetail']=="Add DO") {echo "Stock Terakhir";} else { echo "Stock Bertambah"; } ?></td>
...
jQuery( document ).ready(function( $ ) {
  ...
  $('#approve').live('click',function(e){
    var
      ...
      $.ajax({
        url: "<?php echo base_url();?>notification2/update_stock_act/approve",
        type : 'POST',
        data : data,
