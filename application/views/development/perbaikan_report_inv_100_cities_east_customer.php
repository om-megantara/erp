    ...
    <div class="box box-solid">
      <div class="box-header">
        ...
        <div class="col-md-12" style="overflow-x: auto; padding: 0px;">
          <table class="table table-bordered table-responsive table-main">
            ...
            <tbody>
                <tr>
                    <td><?php echo number_format($content['confirmed'],2);?></td>
                    <td><?php echo number_format($content['completed'],2);?></td>
                    <td><?php echo number_format($content['confirmed']+$content['completed'],2);?></td>
                </tr>
            </tbody>
          </table>
        </div>
      ...
      <div class="box-body">
        <table id="dt_list" class="table table-bordered " style="width: 100% !important;">
          ...
          <?php
            ...
            if (isset($content['main'])) {
                ...
                <tr>
                  ...
                  <td class="alignRight"><?php echo number_format($list['completed']+$list['confirmed'],2);?></td>
                  <td>
                    <button type="button" class="btn btn-primary btn-xs detail" title="DETAIL" id="<?php echo $list['CustomerID']; ?>" datestart="<?php echo $list['INVDateStart']; ?>" dateend="<?php echo $list['INVDateEnd']; ?>" data-toggle="modal" data-target="#modal-detail"><i class="fa fa-fw fa-search"></i></button>
                    <button type="button" class="btn btn-success btn-xs detailmonth" title="DETAIL BY MONTH" id="<?php echo $list['CustomerID']; ?>" datestart="<?php echo $list['INVDateStart']; ?>" dateend="<?php echo $list['INVDateEnd']; ?>" data-toggle="modal" data-target="#modal-detail"><i class="fa fa-fw fa-search"></i></button>
                  </td>
                </tr>
          <?php } } ?>
          </tbody>
          <tfoot>
              <tr>
                  <th colspan="3">Total Page:</br>Total All Page:</th>
...
jQuery( document ).ready(function( $ ) {
  ...
  var table = $('#dt_list').DataTable({
    ...
    "footerCallback": function ( row, data, start, end, display ) {
        ...
        $( api.column( 4 ).footer() ).html(
            pageTotal3.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2}) +'<br>'+
            // '('+ total3.toLocaleString(undefined, {minimumFractionDigits: 2}) +')'
            total3.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2})
        );
        $( api.column( 5 ).footer() ).html(
            pageTotal4.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2}) +'<br>'+
            // '('+ total4.toLocaleString(undefined, {minimumFractionDigits: 2}) +')'
            total4.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2})
        );
        $( api.column( 6 ).footer() ).html(
            pageTotal5.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2}) +'<br>'+
            // '('+ total5.toLocaleString(undefined, {minimumFractionDigits: 2}) +')'
            total5.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2})
        );
    }
  })
  ...
  function get(id, datestart, dateend) {
    xmlHttp=GetXmlHttpObject()
      var url="<?php echo base_url();?>report/report_inv_customer_detail"
  ...
  function get2(id, datestart, dateend) {
    xmlHttp=GetXmlHttpObject()
      var url="<?php echo base_url();?>report/report_inv_customer_detail_month"
      url=url+"?id="+id+"&datestart="+datestart+"&dateend="+dateend
      document.getElementById("detailcontent").innerHTML='<i class="fa fa-spinner fa-spin fa-3x fa-fw"></i><span class="sr-only">Loading...</span>'
      // alert(url);
      xmlHttp.onreadystatechange=stateChanged
