      ...
      <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title">MUTATION Detail</h4>
            </div>
            ...
            <tbody>
              ...
              ?>
                      <tr>
                        ...
                        <td><?php echo $list['Title']." - ".$list['MutationID'];?></td>
                        ...
                        <td>
                          <button type="button" class="btn btn-success btn-xs detail" title="DETAIL" id="<?php echo $list['ApprovalID'];?>" MutationID="<?php echo $list['MutationID'];?>" data-toggle="modal" data-target="#modal-note"><i class="fa fa-fw fa-edit"></i></button>
                        </td>
                      </tr>
              <?php } } ?>
            </tbody>
...
jQuery( document ).ready(function( $ ) {
  ...
  function get(MutationID, id) {
    xmlHttp=GetXmlHttpObject()
      var url="<?php echo base_url();?>approval/approve_mutation_product_detail"
      url=url+"?mutation="+MutationID+"&id="+id
      // alert(url);
  ...
  $('.approve').live('click',function(e){
      MutationID = $(this).attr('mutation');
      id   = $(this).attr('id');
      data = {MutationID:MutationID, id:id};
      var r = confirm("Apa anda yakin akan me-Approve Mutation "+MutationID+"?");
      if (r == true) {
        $.ajax({
          url: "<?php echo base_url();?>approval/approve_mutation_product_act/approve",
          type : 'POST',
          data : data,
  ...
  $('.reject').live('click',function(e){
      ...
      id   = $(this).attr('id');
      data = {MutationID:MutationID, id:id};
      var r = confirm("Apa anda yakin akan me-Reject Mutation "+MutationID+"?");
      if (r == true) {
        $.ajax({
          url: "<?php echo base_url();?>approval/approve_mutation_product_act/reject",
          type : 'POST',
          data : data,
          success : function (response) {
            window.location.href = "<?php echo current_url(); ?>";
